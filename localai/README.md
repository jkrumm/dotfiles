...
> **⚠️ RETIRED 2026-05-25.** This local stack (mlx-audio STT :8000, Fish S2 Pro TTS :8002, localai-helper :8001) is no longer installed by `make setup` and no longer runs. TTS/STT moved to the **cloud audio-gateway** (`~/SourceRoot/audio-gateway`, OpenAI-compatible VPS container at `audio-gateway.jkrumm.com`, reached over the tailnet) which fronts the IU unified audio endpoint. Hermes and MacWhisper now point at the gateway (`https://audio-gateway.jkrumm.com/v1` over the tailnet). Everything in this directory is **kept, not deleted**, so the stack can be re-added later by re-listing `_setup-localai` in the Makefile setup chain. To tear down a machine that still has it loaded: `make localai-teardown`. The rest of this file documents the stack as it was.
