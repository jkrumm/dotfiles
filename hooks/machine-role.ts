  ...
  switch (backend) {
    case "cache":
      return [
        ...
        "non-interactive tool call; run `ask-human.sh ask '…' --cmd 'make secrets-seed'` yourself —",
        "it pushes a native approval dialog to the MacBook right away (the default), the owner",
        "clicks Run, and you get the result back. Never tell the owner a thing 'needs him' or to",
        "drain `make human-queue`; only an unreachable MacBook or unanswered dialog queues it. Which refs the",
        ...
        "`ask-human.sh ask '…' --cmd '…'` (pushes the dialog), which shows the human the exact command first.",
