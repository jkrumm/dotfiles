...
`parse-queue.ts` lives in `cqueue/server/lib/` and is used only by the web server. The `cq` CLI (`scripts/queue.ts` in dotfiles) remains a separate implementation — sharing the parser module is not worth the coupling given they live in different scopes.
