...
only *configures* (links config, runs services, wires the VM). **npm-global and
uv tools stay Makefile-managed** — they need Node/uv on PATH,
not ready that early in bootstrap — so the Brewfile is deliberately brew-native
only.
...
unpatched hypervisor to protect a two-key plist that `make colima-restart`
re-converges anyway. It gets the post-upgrade assertion and a 5-minute health
check instead — the same trade with the failure made loud rather than prevented.
...
**That cask exclusion is a preference, not a guarantee — unlike the caddy pin.** A bare `brew upgrade` upgrades casks too (it says so: *"Homebrew will
...
preference honoured by the command you normally reach for. caddy is pinned
because *its* failure mode is silent config revert, which no amount of care at
the keyboard catches after the fact.

**It asserts rather than assumes the pin protected anything** — a dependency
upgrade can relink a dependent, so after upgrading it re-checks the caddy DNS
module directly (dev-host-gated on the `cache` backend marker, the same gate
`caddy-dns-build` uses) and re-checks that every held package is still pinned. `--dry-run` deliberately does *not* converge pins: a `-dry` target that
