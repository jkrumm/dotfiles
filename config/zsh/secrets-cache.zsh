...
# materializes the `baseline` profile into every new shell's ambient env — the
# same profile the agent host resolves headlessly at runtime. See CLAUDE.md
# "Secrets Strategy" and ~/SourceRoot/dotfiles-private/docs/design.md.
#
...
if [[ "$(cat "$HOME/.config/secrets/backend" 2>/dev/null)" == "cache" ]]; then
  if command -v secrets-run >/dev/null 2>&1; then
    ...
    _secrets_cache_export="$(secrets-run --export baseline 2>"$_secrets_cache_err")"
    ...
    if [[ $? -eq 0 ]]; then
      ...
      echo "! secrets-cache: failed to load baseline secrets — run 'secrets-run --profile baseline -- true' to see why" >&2
      [[ -s "$_secrets_cache_err" ]] && cat "$_secrets_cache_err" >&2
    fi
