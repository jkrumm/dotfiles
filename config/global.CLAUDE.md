...
| `hermes-agent` | Hermes — mini-only personal AI (Slack interface, `gpt-5.6-luna` brain via the IU endpoint with `claude-sonnet-4-6-eu` failover). `HERMES_SKILLS` in its Makefile is the source of truth for its skill domains — don't restate the count here, it drifts. |
